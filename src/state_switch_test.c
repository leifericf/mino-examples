...
static void test_interleaved_eval(void)
{
    ...
    mino_val *r1 = mino_eval_string(s1, "@counter", e1);
    mino_val *r2 = mino_eval_string(s2, "@counter", e2);
...
static void test_cross_state_value_safety(void)
{
    ...
    mino_val *v = mino_eval_string(s1, "{:data [1 2 3] :name \"test\"}", e1);
    ...
    mino_val *r = mino_eval_string(s2, "(reduce + 0 (get imported :data))", e2);
...
static mino_val *prim_check_state(mino_state *S, mino_val *args,
                                     mino_env *env)
{
    (void)args; (void)env;
...
static void test_current_state_in_primitive(void)
{
    ...
    mino_val *r1 = mino_eval_string(s1, "(check-state)", e1);
    mino_val *r2 = mino_eval_string(s2, "(check-state)", e2);
    CHK(r1 && r2, "eval failed");
    long long v1, v2;
...
static void test_env_clone_shared_atoms(void)
{
    ...
    mino_val *r = mino_eval_string(S, "@shared-atom", env);
...
static void test_gc_multi_env_roots(void)
{
    ...
    mino_state *S = mino_state_new();
    mino_env *env1 = mino_env_new_default(S);
    mino_env *env2 = mino_env_new_default(S);
    int i;
    ...
    mino_val *v = mino_eval_string(S, "big-vec", env1);
    ...
    mino_val *r = mino_eval_string(S, "(count shared-vec)", env2);
    CHK(r != NULL, "eval failed");
    long long val;
    CHK(mino_to_int(r, &val), "not int");
...
static void test_eval_string_multi_form_error(void)
{
    ...
    mino_clear_error(S);
    mino_val *a_val = mino_eval_string(S, "a", env);
    CHK(a_val != NULL, "a should exist");
    mino_clear_error(S);
    mino_val *c_val = mino_eval_string(S, "c", env);
...
static void test_call_user_fn(void)
{
    ...
    mino_val *args = mino_cons(S, mino_int(S, 3),
                       mino_cons(S, mino_int(S, 4),
                       mino_cons(S, mino_int(S, 5), mino_nil(S))));

    mino_val *r = mino_call(S, fn, args, env);
...
static void test_multiple_repls(void)
{
    ...
    mino_state *S = mino_state_new();
    mino_env *env = mino_env_new_default(S);
    mino_repl *r1 = mino_repl_new(S, env);
    mino_repl *r2 = mino_repl_new(S, env);
    mino_val *out = NULL;
